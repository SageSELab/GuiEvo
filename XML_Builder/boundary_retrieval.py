...
def get_boundaries(info_dict, xmltree):
	...
	for i in range (len(xmltree)):
		if xmltree[i][2]!=None and xmltree[i][4]["bounds"]!='[0,0][0,0]' and xmltree[i][4]["bounds"]!='[0,0][1080,63]':
			...
			info_dict[xmltree[i][0]].update({'component_validity':'valid','old_bounds': xmltree[i][4]["bounds"],'new_bounds': xmltree[i][4]["bounds"],'total_changes':tc,'change_types': "NC",'new_attributes': xmltree[i][4]})
			
		elif xmltree[i][2]!=None and (xmltree[i][4]["bounds"]=='[0,0][0,0]' or xmltree[i][4]["bounds"]=='[0,0][1080,63]'):
			info_dict[xmltree[i][0]].update({'component_validity':'ignored','old_bounds': xmltree[i][4]["bounds"],'new_bounds': xmltree[i][4]["bounds"],'total_changes':tc,'change_types': "NC",'new_attributes': xmltree[i][4]})
